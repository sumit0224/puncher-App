...
export default function VendorLogin() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response: any = await api.post('/auth/login/vendor', formData);

            localStorage.setItem('token', response.token);
            // Construct vendor object from response, typically backend sends id, name, etc.
        ...
        >
            <div className="space-y-6 text-black">
                ...
                <div className="text-center text-sm">
                    <span className="text-gray-500">Want to join our network? </span>
                    <Link href="/register/vendor" className="font-bold text-black hover:text-yellow-500 hover:underline">
                        Register Shop
                    </Link>
                </div>
            </div>
